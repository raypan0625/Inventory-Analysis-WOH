...
season<-data.frame(c(0.0350,0.0499,0.0997,0.1519,0.1730,0.1198,0.0985,0.0796,0.0678,0.0471,0.0469,0.0307))
a=m+1
b=m+2
c=m+3
d=m+4
...
woh<-assortment %>%
  select(Picture, stockkey, `HMNum 1`,`HMNum 2`,`HMNum 3`, `Product Name`,Tier,`New Category3-2`,`First Receiving Date`,Unit,MP,`Latest Factory Name`, "Country of Factory"=`Representative Country of Factory`,`Units 40' HQ`,  `Last First Cost` )
...
`Weekly Sales_OOS_SEA (Aug-Dec)`<- rep("", nrow(woh))
...
CNforecast_ty = 1/2*season[1,]+season[11,]+season[12,]
CNforecast_ny = season[2,]+1/2*season[3,]
...
```{r} 
#extract as excel
write_xlsx(woh,"C:\\Users\\Yulin Pan\\Desktop\\Outdoor WOH old.xlsx")
```
